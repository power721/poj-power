...
Date.prototype.format = function (format) {
  ...
  var k = '';
  if (/(y+)/.test(format)) {
    format = format.replace(RegExp.$1, String(this.getFullYear())
...
var extArray = [".in", ".out", ".txt", ".doc", ".c", ".cc", ".java",
...
function checkUsername() {
  ...
  var patrn = /^[0-9_A-Za-z]{5,20}$/;
  if (!patrn.test(userId)) {
    $("#user_id").focus();
    alert("用户名长度应该在5到20之间, 只能包含字母、数字和_(下划线)。");
    return false;
  }
  return true;
}
