...
public class Status extends HttpServlet
{
	...
	private static String _$6092 = "<TABLE cellSpacing=0 cellPadding=0 align=center width=99% border=1 background=images/table_back.jpg style=\"border-collapse: collapse\" bordercolor=#FFFFFF><tr bgcolor=#6589D1><td class=status align=center width=8%><b>Run ID</b></td><td class=status align=center width=10%><b>User</b></td><td class=status align=center width=6%><b>Problem</b></td><td class=status align=center width=20%><b>Result</b></td><td class=status align=center width=7%><b>Memory</b></td><td class=status align=center width=7%><b>Time</b></td><td class=status align=center width=7%><b>Language</b></td><td class=status align=center width=7%><b>Code Length</b></td><td class=status align=center width=17%><b>Submit Time</b></td></tr>\n";
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		try
		{
			if (l != 0L)
			{
				...
				if (!resultset.next())
				{
					FormattedOut.printContestHead(out, l, "Problem Status List", request);
		...
		if (j > 500)
			j = 500;
		String s8 = "solution_id,user_id,problem_id,result,in_date,language,memory,time,contest_id ,num,code_length as len from solution s ";
		boolean flag1 = false;
		String s9 = "";
		if (!s.trim().equals(""))
		{
		...
		{
			if (!flag1)
				s8 = s8 + " where s.contest_id is NULL or (select 1 from contest c where c.contest_id=s.contest_id and c.private=0) ";
			else
				s8 = s8 + " and (s.contest_id is NULL or (select 1 from contest c where c.contest_id=s.contest_id and c.private=0)) ";
		}
		...
		// s8 = "SELECT " + s8 + "order by solution_id " + (k != 0 ? "" :
		// "desc") + " limit " + j;
		if (p < 1)
			p = 1;
		...
		try
		{
			...
			if (s1 == null)
				FormattedOut.printHead(out, request, connection1, "Problem Status List");
			else
				FormattedOut.printContestHead(out, l, "Problem Status List", request);
			...
			out.print("<p align=center><font size=5 color=blue>Problem Status List</font></p>");
			out.print("<div id=status><form method=get action=status>" + (l != 0l ? ("<input type=hidden name=contest_id value=" + l + ">") : "") + "Problem ID:<input type=text name=problem_id size=8 value=\"" + (s != "" && p_id != pid ? "" + (char) (pid + 65) : s) + "\">User ID:<input type=text name=user_id size=15 value=\"" + s3 + "\">Result:<select size=1 name=result><option value='' " + (l1 != -1 ? "" : " selected") + ">All</option><option value=0" + (l1 != 0 ? "" : " selected") + ">Accepted</option><option value=1" + (l1 != 1 ? "" : " selected") + ">Presentation Error</option><option value=2" + (l1 != 2 ? "" : " selected") + ">Time Limit Exceeded</option><option value=3" + (l1 != 3 ? "" : " selected") + ">Memory Limit Exceeded</option><option value=4"
			...
			if (resultset.next())
			{
				...
				if(pageall < 1)
					pageall = 1;
				//System.out.println("records:"+resultset.getInt("page")+" size:"+j+" pages:"+pageall+" sql:"+sqlall);
			}
			...
			if (flag5 && resultset1.first() || !flag5 && resultset1.last())
				do
				{
					...
					if ((flags1 && flags2) || flag3 || flag4 || UserModel.isUser(request, s10))
						out.print("<td><a href=showsource?solution_id=" + i + " target=_blank>" + s14 + "</a></td>");
					else
						out.print("<td>" + s14 + "</td>");
					if (flag8)
		...
		out.print("</table></div>\n<p align=center>[<a href=status" + s9 + ">Top</a>]&nbsp;&nbsp;");
		if (flag2)
			s9 += "&";
		else
			s9 += "?";
		if (p > 1)
			out.print("[<a href=status" + s9 + "p=" + (p - 1) + "><font color=blue>Previous Page</font></a>]&nbsp;&nbsp;");
		else
			out.print("[Previous Page]&nbsp;&nbsp;");
		if (p < pageall)
			out.print("[<a href=status" + s9 + "p=" + (p + 1) + "><font color=blue>Next Page</font></a>]&nbsp;&nbsp;");
		else
			out.print("[Next Page]&nbsp;&nbsp;");
		out.print("[<a href=status" + s9 + "p=" + pageall + "><font color=blue>Last Page</font></a>]&nbsp;&nbsp;" + p + " of " + pageall + "</p>\n");
